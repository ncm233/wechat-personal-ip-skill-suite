...
In this Codex workspace, this skill is installed as an optional layout style provider for `wechat-article-pipeline` and `wechat-layout`.
...
The local pipeline uses this skill's `styles/custom.css` and bm.md rendering style, but draft-box publishing remains delegated to the existing `wechat-publish-draft` skill. Do not use this imported skill's standalone credential names or publishing flow unless the user explicitly asks to run it outside the local pipeline.

Local formatter command used by the pipeline:

```bash
python C:\Users\ZhuanZ（无密码）\.codex\skills\wechat-layout\scripts\format_bm_md_article.py \
  --input "<rewritten.md>" \
  --output-dir "<article-folder>" \
  --basename article
```
